...
# evaluate.sh wrapper for llm_router
PROBLEM_DIR=$(pwd)
echo "[evaluate] Running LLM_ROUTER evaluation..." >&2
echo "[evaluate] Current dir: $(pwd)" >&2

# Try to run the evaluator, capture output
if ./run_evaluator.sh 2>&1 | tee /tmp/eval_output_llm_router.log; then
    echo "[evaluate] run_evaluator succeeded" >&2
else
    echo "[evaluate] ERROR: run_evaluator.sh failed!" >&2
    echo "[evaluate] Output:" >&2
    cat /tmp/eval_output_llm_router.log >&2
    echo "-10000"
    exit 0
fi
